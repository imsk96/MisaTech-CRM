...
async function renderDispatchesPage(container) {
    ...
    const [dispatches, staff] = await Promise.all([
                ...
                <tbody>
                    ${dispatches.map(dispatch => `
                        <tr>
                            ...
                            <td>${dispatch.dispatch_details.details || 'N/A'}</td>
...
function showManualDispatchModal(staffList) {
    ...
    const formHTML = `
        <form id="manual-dispatch-form">
            ...
            <div class="input-group"><textarea name="dispatch_details" placeholder="Enter product name, quantity, etc."></textarea><label>Dispatch Details</label></div>
